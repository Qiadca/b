                ...
                @foreach($project_items as $row)
                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="project-item wow fadeInUp mb_30">
                            <div class="photo">
                                <a href="{{ url('araclarimiz/'.$row->project_slug) }}"><img src="{{ asset('public/uploads/'.$row->project_featured_photo) }}" alt=""></a>
                            </div>
                            <div class="text">
                                <h3><a href="{{ url('araclarimiz/'.$row->project_slug) }}">{{ $row->project_name }}</a></h3>
                                <p>{{ $row->project_content_short }}</p>
                                <div class="read-more">
                                    <a href="{{ url('araclarimiz/'.$row->project_slug) }}">Devamını Oku</a>
                                </div>
                            </div>
                        </div>
                    </div>
