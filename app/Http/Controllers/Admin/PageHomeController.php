...
class PageHomeController extends Controller
{
    ...
    {
        if($request->hasFile('appointment.bg'))
        {
            $request->validate([
                'appointment.bg' => 'image|mimes:jpeg,png,jpg,gif|max:2048'
            ]);
            ...
            // Uploading new photo
            $ext = $request->file('appointment.bg')->extension();
            $final_name = 'appointment.bg'.'.'.$ext;
            $request->file('appointment.bg')->move(public_path('uploads/'), $final_name);

            $data['appointment.bg'] = $final_name;
        }
		if($request->hasFile('appointment.bg'))
        {
            $request->validate([
