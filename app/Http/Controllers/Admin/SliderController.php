...
class SliderController extends Controller
{
    ...
    {
        $request->validate([
            'slider_photo_mb' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);
        ...
        $ext = $request->file('slider_photo_mb')->extension();
        $final_name = 'slider-'.$ai_id.'.'.$ext;
        $request->file('slider_photo_mb')->move(public_path('uploads'), $final_name);

        $slider = new Slider();
        $data = $request->only($slider->getFillable());

        unset($data['slider_photo_mb']);
        $data['slider_photo_mb'] = $final_name;
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('slider_photo')) {
		   ...
		   if ($request->hasFile('slider_photo_mb')) {

            $request->validate([
                'slider_photo_mb' => 'image|mimes:jpeg,png,jpg,gif|max:2048'
            ]);

            unlink(public_path('uploads/'.$slider->slider_photo_mb));
